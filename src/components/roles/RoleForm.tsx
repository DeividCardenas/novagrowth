    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <button type="button" onClick={() => setPermissions([])} className="text-xs underline">Limpiar</button>
        </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                {ACTIONS.map(a => <th key={a} className="p-3 capitalize font-semibold">{a}</th>)}
              </tr>
            </thead>
            <tbody>
              {MODULES.map(m => (
                <tr key={m} className="odd:bg-white even:bg-gray-50">
                  <td className="p-3 font-medium capitalize">{m}</td>
                  {ACTIONS.map(a => {
                    const checked = permissions.some(p => p.module === m && p.action === a);
                    return (
                      <td key={a} className="p-3 text-center">
                        <input type="checkbox" className="h-4 w-4" checked={checked} onChange={() => togglePermission(m, a)} />
                      </td>
                    );
                  })}
                </tr>
      ...
      <div className="flex items-center gap-3">
        <Button type="submit" disabled={saving}>{saving ? "Guardando…" : (initial ? "Actualizar" : "Crear")}</Button>
